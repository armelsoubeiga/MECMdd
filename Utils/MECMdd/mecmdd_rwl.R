mecmdd.rwl<-function(Xlist, c, type='full', alpha=1, beta=1.5, delta=9, epsi=1e-4, 
  ...
  lambda <- lambdaInit_local(weight, pmatrix, nF-1)
    ...
    for(j in 1:(nF-1)){
      ...
      for(i in 1:n){
        num <- (card[j]^alpha * sum(sapply(seq_along(Xlist), 
                      function(l) Xlist[[l]][i,prototype[j]] * (lambda[j,l])^s)))^(-1/(beta-1))
        den <- ((sum(sapply(seq(nF-1), function(k) 
            (card[k]^alpha * sum(sapply(seq_along(Xlist), 
              function(l) Xlist[[l]][i,prototype[k]] * (lambda[k,l])^s)))^(-1/(beta-1))))) + delta2^(-1/(beta-1)))
        m[i,j] <- num/den
      ...
      for(j in 1:(nF-1)){
        for(l in 1:pmatrix){
          num <- prod(sapply(seq_along(Xlist), function(h)
            sum(sapply(seq(n), function(i) 
                (card[j]^alpha) * m[i,j]^beta * Xlist[[h]][i,prototype[j]]))^(1/pmatrix)),na.rm = TRUE)
      ...
      if(sum(fk) == 1) {
        ...
        for(i in 1:n){
          V[i,k] <- sum(rowSums(sapply(seq_along(Xlist), 
                     function(l) Xlist[[l]][i,] * (lambda[k,l])^s))*mk_beta)
        }
      }
    }
    ...
    J <- sum(sapply(seq(n), function(i) 
      sum(sapply(seq(nF-1), function(j) card[j]^alpha * m[i,j]^beta * 
             sum(sapply(seq_along(Xlist), 
                    function(l) Xlist[[l]][i,prototype[j]] * (lambda[j,l])^s)))))) +
